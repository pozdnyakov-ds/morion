<template>
    <v-navigation-drawer 
        ...
        v-model="indexStore.displayAdd.visible" 
        text-color="white"
    >
    <div>
        <div class="page-title" style="padding: 20px; margin: 0px; background-color: #9ce9bf;">Добавить дисплей</div>
        <YandexMap :key="indexStore.display.key"/>
            ...
            <v-form v-model="valid">
                ...
                <v-card-actions style="margin-bottom: 20px;">
                    <v-spacer></v-spacer>
                    <v-btn color="green-darken-1" variant="text" @click="submitForm(recordInfo)" :disabled="!valid || !group.selected || !layout.selected || addressValid != 1">Создать</v-btn>
...
const submitForm = async (recordInfo) => {
	...
	try {
		...
		if (data.value && data.value.code == 200) {
            ...
            indexStore.displayAdd.reload = randomId(32)
...
const dialog_action_cancel = () => {
    indexStore.displayAdd.visible = false
}
