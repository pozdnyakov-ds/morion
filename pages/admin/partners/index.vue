...
<div class="main-content">
    ...
    <v-row>
        <v-col>
            <nuxt-link to="/admin/partners/create">
    ...
    <template v-slot:item.cost="{ item }">
        <div>{{ item.cost && item.cost != '0.00' ? "-" + item.cost : '0.00' }}</div>
    </template>

    <template v-slot:item.summary="{ item }">
        <div>{{ (item.wallet*1.0 + item.income*1.0 - item.cost*1.0).toFixed(2) }}</div>
    </template>
...
const dialog_edit_yes = async () => {
	...
	router.push({ path: `/admin/partners/${dialog.action}` })
}
